(function() {
  ...
  App.service("DropboxBackupService", ["$http", "toastr", "CatchHttpError", "$q", function($http, toastr, CatchHttpError, $q) {
    this.get = function() {
  ...
  App.controller("DropboxBackupPluginCtrl", function($scope, DropboxBackupService, SettingsSavedToastr, toastr, CatchHttpError, $timeout, $ngConfirm) {
    ...
    $scope.updateSettings = function() {
      return DropboxBackupService.update({
        ...
        time: $scope.time
      }).then(SettingsSavedToastr).catch(CatchHttpError)
    };
